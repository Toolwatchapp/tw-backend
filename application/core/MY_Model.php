...
class MY_Model extends CI_Model {
	 ...
	 * @access protected
	 */
	protected $before_union_all = array();
	 ...
	 * @access protected
	 */
	protected $after_union_all = array();
	 ...
	 * @var array of select Strings
	 */
	protected $pending_selects = array();

	//--------------------------------------------------------------------

	/**
	 * FF_Model's constructor
	 *
	 ...
	 * false on failure or empty set.
	 */
	public function find_union_all() {
		$this->trigger('before_union_all');

		$query_string = "";

		$last_select = count($this->pending_selects)-1;
		for ($i = 0; $i < $last_select; $i++) {
			$query_string = $query_string."(".$this->pending_selects[$i].")  union all ";
		}

		$query_string = $query_string."(".$this->pending_selects[$i].")";
		...
		if (is_array($return)) {
			$last_record = count($return)-1;
			foreach ($return as $key => &$row) {
				$row = $this->trigger('after_union_all', $row, ($key == $last_record));
			}
		}
		...
		unset($this->pending_selects);
		$this->pending_selects = array();
	 ...
	 */
	public function update_table($where = null, $data = null, $table = null) {
		if ($this->skip_validation === false) {
	 ...
	 * @return bool True on successful update, else false
	 */
	public function increment($where = null, $field = null) {
		if (!is_array($where)) {
			$where = array($this->key => $where);
		}
	 ...
	 * @return bool True on successful update, else false
	 */
	public function decrement($where = null, $field = null) {
		if (!is_array($where)) {
			$where = array($this->key => $where);
		}
	...
	public function get_field($id = null, $field = '') {
		if (empty($id) || empty($field)) {
			$this->error = lang('ff_model_fetch_error');
			$this->logit('['.get_class($this).': '.__METHOD__ .'] '.lang('ff_model_fetch_error'));
	 ...
	 *
	 * @return FF_Model An instance of this class.
	 */
	 ...
	 *
	 * @return FF_Model An instance of this class to allow method chaining
	 */
	...
	public function union($select) {array_push($this->pending_selects, $select);return $this;}
