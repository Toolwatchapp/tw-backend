		...
		if ($this->input->post('token') === $this->api_key) {
			...
			if (startsWith($text, "Jack nbusers")) {
				...
				$user = $this->user->select(" user.userId, user.name, firstname,
                    ...
                    DATE_FORMAT(FROM_UNIXTIME(`lastLogin`), '%e %b %Y') AS 'lastLogin', facebook, country", false)
				->find_by('email', $text);
				...
				if (is_object($user)) {
					...
					$result["text"] = "ID;Name;Firstname;register;lastLogin;facebook_user;country \r\n".
					"```". $user->userId . ";" . $user->name . ";" . $user->firstname .
					 ";" . $user->register . ";" . $user->lastLogin . ";" . $user->facebook . ";" . $user->country . "```\r\n"
					 . "Dashboard\r\n"
